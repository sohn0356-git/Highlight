 ...
 * - Also handles yesterday/fallback dates
 */
...
async function fetchQTPage(dateStr) {
  // duranno.com uses date format like "2026-9-1" (no leading zeros)
...
function parseQT(html, dateStr) {
  ...
  // Extract passage (e.g., "시편 119:1~16")
  let passage = "";
  const passageEl = $("span.info_bible").first();
  if (passageEl.length) {
    passage = passageEl.text().trim();
  }
  // Fallback: look for passage pattern in the content
  if (!passage) {
    const match = html.match(/((?:시편|창세기|출애굽기|레위기|민수기|신명기|여호수아|사사기|루트|사무엘|열왕기|역대|에스라|느헤미야|에스더|욥기|잠언|전도서|아가|이사야|예레미야|에스겔|다니엘|호세아|요엘|아모스|오바디야|요나|미가|나훔|하바까크|스바니야|학개|스가랴|마태복음|마가복음|루가복음|요한복음|사도행전|로마|고린도|갈라디아|에베소|빌립보|골로새|데살로니가|디모데|디도|레몬|히브리|야고보|베드로|요한|유다|요한계시록)\s*\d+\s*:\s*\d+[~\-]\d+)/);
  ...
  if (bibleDiv.length) {
    content = bibleDiv.text().trim().replace(/\s+/g, " ");
  }
  ...
  // Extract questions (QT reflection questions)
  let question1 = "";
  let question2 = "";
  const qEls = $("p.mat_q, .meditation_q");
  if (qEls.length >= 1) question1 = $(qEls[0]).text().trim();
  if (qEls.length >= 2) question2 = $(qEls[1]).text().trim();
  
  // Fallback questions from content
...
async function upsertQT(qtData) {
  ...
  const { data, error } = await sb.from("qt_today").upsert(
    {
      date: qtData.date,
  ...
  if (error) {
    ...
    // Try insert if upsert fails (table might not have date as unique)
    const { error: insertError } = await sb.from("qt_today").insert([{
      date: qtData.date,
      passage: qtData.passage,
      verse: qtData.verse,
      content: qtData.content,
  ...
  console.log(`✅ QT for ${qtData.date} saved successfully`);
  return true;
}
...
async function main() {
  ...
  try {
    ...
    console.log(`📄 Content length: ${qtData.content.length} chars`);
    console.log(`🙏 Prayer length: ${qtData.prayer.length} chars`);
    
    await upsertQT(qtData);
    
    // Also create/update admin QT content record
    const adminQT = {
      id: `qt_${dateStr}`,
      date: dateStr,
      title: qtData.verse || qtData.passage,
      passage: qtData.passage,
      verse: qtData.verse,
      content: qtData.content,
      question1: qtData.question1 || "오늘 말씀에서 나를 향한 하나님의 뜻은 무엇인가요?",
      question2: qtData.question2 || "이 말씀을 어떻게 내 삶에 적용할 수 있을까요?",
      mileageReward: 20,
      active: true,
      status: "active",
    };
    
    console.log(`\n✅ QT update complete for ${dateStr}`);
    
  } catch (err) {
    console.error(`❌ Error updating QT: ${err.message}`);
    process.exit(1);
  }
}
